...
void ayan(){
/*
 1.  first we handle all i's where inv1[i] > inv2[i]; and add them to answer
 2.  at the same loop we calculate the (dummy)i for which inv2[i]- inv1[i] is max
 3.  Then we calc all diffs where inv1[i]<inv2[i]; store them and sort them
 4.  and then we count how many diff's can handled 
  	by dummy provided dummy > 0;
*/
